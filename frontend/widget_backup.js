...
function formatINR(number) {
    if (number >= 10000000) {
        let val = (number / 10000000).toFixed(2);
        let formatted = parseFloat(val).toString();
        return `₹${formatted} Crores`;
    } else if (number >= 100000) {
        let val = (number / 100000).toFixed(2);
        let formatted = parseFloat(val).toString();
...
function queryProperty(propId) {
    const propertyNames = {
        ...
        "prop_006": "Emami Aastha Bungalow",
        "prop_007": "Emami Business Bay",
        "prop_008": "Merlin - The Summit"
    };
...
function renderCitationRow(botBubble, properties) {
    ...
    properties.forEach(prop => {
        ...
        card.innerHTML = `
            ...
            <div class="mini-prop-price">${formatINR(prop.price)}</div>
